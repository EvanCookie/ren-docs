# 关于Git
![img](./assets/git_base/git.jpeg){.lg-thumbnail}
...
![img](./assets/git_base/1710831903075-ca64b740-e22d-404b-8518-d8ec573b8493.png)
...
![img](./assets/git_base/1711160360657-6b3abc98-5249-4df2-bf0d-30cca4d2278a.png)
## 7. 查看仓库状态
...
![img](./assets/git_base/1711693522346-74e6707e-2547-489c-918e-2a6d0778e849.png)
...
![img](./assets/git_base/1711770939117-10146d85-1467-4583-a01f-49ddfd9e8134.png)

**第二步：** 创建 test 分支，随后在 test 分支创建一个 c 文件，随后提交到 test 分支。
![img](./assets/git_base/1711777728665-2eafdf47-b07b-493d-be42-934443421d25.png)

**第三步：** 切回 master 分支。

此时发现工作区中 c 文件消失（因为 master 分支中没有 c 文件）。
![image.png](./assets/git_base/1711771228930-9b6827f8-5389-40f8-894f-53fee037e5ea.png)

**第四步：** 把 test 分支合并到 master 分支
使用`git merge test`合并分支，合并成功后，会发现工作区中已经有了 c 文件，且可以查看到 c 文件的提交记录。
![image.png](./assets/git_base/1711777434203-3a837743-cd6d-430f-8b67-b71ea9e82405.webp)
:::
...
![image.png](./assets/git_base/1711804705867-ce4e6034-dfba-4951-b1bc-2e6920fcf6a1.webp)

**第五步**：在 master 分支中合并 test 分支
输入`git merge test`命令，进行分支合并
![img](./assets/git_base/1711805046418-ff04098c-4b39-4854-816f-0fc425fda10c.webp)

出现如下提示，含义是本次分支合并，存在不一样的版本，请为生成的新版本编写说明
![img](./assets/git_base/1711805343244-d7271863-ac0c-4c7e-b983-8a935cd88bc1.png)

输入完说明后，保存并退出

![img](./assets/git_base/1711805410686-75568a3d-a35b-4c40-a72b-6a97741bca9b.png)

随后提示合并完毕

![img](./assets/git_base/1711805476204-b30359f9-adeb-4c83-bb0a-010cf3b38798.webp)
:::
...
![img](./assets/git_base/1711778784216-1c2d8b56-55a9-403f-9961-d109cf3dd743.png)
...
![img](./assets/git_base/1712634803620-3f65f793-ab1c-4d12-b4df-9272ad68ecaf.gif)

- 变基（rebase） 使用 `git rebase master` 命令

将一个分支（test）上的提交重新应用到另一个分支（master）上，变基会重写项目历史，因为它实际上是在创建一系列新的提交，会产生一个更线性的历史记录，看起来更干净、更简单。

![img](./assets/git_base/1712278910007-278e9284-cae4-46d7-b32b-89810a7f1359.gif)
...
![image-20240721152359852](./assets/git_base/image-20240721152359852.png)

### 14.8. 游离分支

`git checkout`也可以将代码签出到指定版本，即可以执行`git checkout 具体版本号`，当签出到指定版本时，签出的代码出于一个游离分支中，如下图操作：

![img](./assets/git_base/1712632266329-1400e3ea-b26d-464c-ae17-88470fe9ea02.png)
...
![img](./assets/git_base/1710828311788-0bd8a49c-99f3-4991-a9ae-593ac0d8d7bc.png)
